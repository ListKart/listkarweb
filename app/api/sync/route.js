...
// Lazy-initialize the SQL client to prevent top-level crashes
function getSql() {
...
async function shareList(sql, body) {
  const { list_name, device_id } = body;
  const sharing_code = Math.floor(100000 + Math.random() * 900000).toString();
  const list_rows = await sql('INSERT INTO shared_lists (sharing_code, list_name) VALUES (, ) RETURNING id', [sharing_code, list_name]);
...
async function inspectList(sql, searchParams) {
  const sharing_code = searchParams.get('sharing_code');
  const rows = await sql('SELECT id, list_name FROM shared_lists WHERE sharing_code = ', [sharing_code]);
  if (rows.length === 0) return NextResponse.json({ error: 'List not found' }, { status: 404 });
  const list = rows[0];
  const count_rows = await sql('SELECT COUNT(*) FROM shared_list_items WHERE shared_list_id = ', [list.id]);
  return NextResponse.json({ success: true, id: list.id, list_name: list.list_name, item_count: parseInt(count_rows[0].count) });
}

async function joinList(sql, body) {
  ...
  const items = await sql('SELECT * FROM shared_list_items WHERE shared_list_id =  ORDER BY order_index', [list.id]);
  return NextResponse.json({ success: true, id: list.id, list_name: list.list_name, items });
}

async function getUpdates(sql, searchParams) {
  const list_id = searchParams.get('list_id');
  const items = await sql('SELECT * FROM shared_list_items WHERE shared_list_id =  ORDER BY order_index', [list_id]);
  return NextResponse.json({ success: true, items });
}

async function syncItems(sql, body) {
