...
async function shareList(sql: any, body: any) {
  ...
  const rows: any = await sql('INSERT INTO shared_lists (sharing_code, list_name) VALUES (, ) RETURNING id', [sharing_code, list_name]);
  const list_id = rows[0].id;
  if (device_id) await sql('INSERT INTO shared_list_participants (shared_list_id, device_id) VALUES (, ) ON CONFLICT DO NOTHING', [list_id, device_id]);
...
async function inspectList(sql: any, searchParams: URLSearchParams) {
  ...
  const list = rows[0];
  const cr: any = await sql('SELECT COUNT(*) FROM shared_list_items WHERE shared_list_id = ', [list.id]);
  return NextResponse.json({ success: true, id: list.id, list_name: list.list_name, item_count: parseInt(cr[0].count) });
}
